-- =========================================================
-- Retro 70s-Style TODO CLI V.3 (Persistent with File Storage)
...
main = do
  ...
  bootAnimation
  tasks <- loadTasks                 -- Load from file at startup
  menuLoop tasks
...
saveTasks :: [String] -> IO ()
saveTasks tasks = writeFile taskFile (unlines tasks)
...
menuLoop tasks = do
  ...
  case choice of
    "1" -> do tasks' <- addTask tasks
              saveTasks tasks'
              menuLoop tasks'
    "2" -> viewTasks tasks >> menuLoop tasks
